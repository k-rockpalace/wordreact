...
const Flatdesk = (props)=>{
    const genreFilterData = props.productData.filter((data)=>data.dataGenre === props.genreData);
    return (
        <section id="page">
			<h2 className="page__heading">{props.genreName}</h2>
		<ul className="page__contents">
            {genreFilterData && genreFilterData.map((data,index)=>(
